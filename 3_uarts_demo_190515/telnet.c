...
static tTelnetSessionData g_sTelnetSession[3];
...
//! \return None.
//
//uint8_t counter_filter=0;
//*****************************************************************************
static void
TelnetProcessCharacter(uint8_t ui8Char, tTelnetSessionData *pState)
{
	uint8_t pui8Buf[9],counter;
	//    UARTprintf("f:0x%02X mst:%d \n",g_sParameters.sPort[pState->ui32SerialPort].ui8Flags,modbusState);

	if(((g_sParameters.sPort[pState->ui32SerialPort].ui8Flags &
			PORT_FLAG_PROTOCOL) == PORT_PROTOCOL_RAW) //raw data;
			&& ((g_sParameters.sPort->uiTCPServerType & 0xF) == 0x1) //transparency;
	)

	{
		if(
				//	(g_sParameters.modbus == Modbus_en) ||
				((g_sParameters.sPort[pState->ui32SerialPort].ui8Flags &
						PORT_PROTOCOL_MODBUS) == PORT_MODBUS_RTU_TCP)
		)//modbus_en;
			// add modbus statemachine;
		{
			...
			{
				//    	    	UARTprintf("0x%02X ",ui8Char);
				ModbusTCPReceiveSerialSend(0, ui8Char); //send to UR port;
			...
			return;
				...
				//	(g_sParameters.modbus == Modbus_en) ||
				((g_sParameters.sPort[pState->ui32SerialPort].ui8Flags &
						PORT_PROTOCOL_MODBUS) == ModbusRTU_RTU)
		)//modbus_en;
		{
			//    		counter_filter++;
			//    		UARTprintf("\n0x%X ",ui8Char);
			//    		UARTprintf("0x%02X ",ui8Char);
			//    		if (counter_filter>=3)
			//    		ModbusPDU_Req_LAN_Port(ui8Char);
...
uint32_t chartodigit(char*IPstring)
{
...
UDPHandler(void)
{
	UARTprintf("\n udpHandler \n");
	struct pbuf *p;
	uint8_t pui8Data_Rev[256],*pui8Data;
	uint32_t ui32Idx,ipAddr_tmp32bit,rev_length;
	ip_addr_t *piphost;
	ip_addr_t iphost;
	// 	iphost.addr = 0x6401a8c0; //0x6401a8c0
	ipAddr_tmp32bit = 0;
	rev_length = 0;
	/*
    sscanf(g_sParameters.sPort->uiTelnetURL, "%3d.%3d.%3d.%3d", &iptemp[0],
    &iptemp[1], &iptemp[2], &iptemp[3]);

    for(i=0;i<4;i++)
    	ipAddr_tmp32bit = ipAddr_tmp32bit<<(i*8)+iptemp[i];
	 */
	ipAddr_tmp32bit = chartodigit((char *)g_sParameters.sPort->uiTelnetURL);
	//   UARTprintf("URL adrr_dest:%s\n",g_sParameters.sPort->uiTelnetURL);
	//   UARTprintf("SSCANFip adrr_dest:%x\n",ipAddr_tmp32bit);

	//	iphost.addr = IPV4_Addr_Make(g_sParameters.sPort->ui32TelnetIPAddr);
	iphost.addr = IPV4_Addr_Make(ipAddr_tmp32bit);
	piphost = &iphost;
	if((Switch_LAN_to_COM ==1 )&& SerialReceiveAvailable(COM_Port))
	{
		// get UDP data from buffer and put into COM ringbuffer;

		// get the COM ringbuffer data and send to UDP;

		// Send_response_data(pcb,p,addr,port,Response_data_length,response_data);



		// Copy the response packet data into the pbuf.

		//	 UARTprintf("send to LAN UDP port:\n ");
		for(ui32Idx = 0; (ui32Idx < 256)&&(SerialReceiveAvailable(COM_Port)); ui32Idx++)
		{
			pui8Data_Rev[ui32Idx] = (uint8_t)SerialReceive(COM_Port);
			//		 UARTprintf("%c",pui8Data_Rev[ui32Idx]);
			rev_length++;
		}

		//	 UARTprintf("\n ");
		//	 UARTprintf("rev_length:%d\n ",rev_length);
		// Allocate a new pbuf for sending the response.
		//		 UARTprintf("allocate p buffer!\n");
		p = pbuf_alloc(PBUF_TRANSPORT, rev_length, PBUF_RAM);
		if(p == NULL) 	 {  return; }


		// Copy the response packet data into the pbuf.
		pui8Data = p->payload;
		for(ui32Idx = 0; ui32Idx < rev_length; ui32Idx++)
		{
			//	 pui8Data[ui32Idx] = pui8Data_Rev[rev_length-ui32Idx-1];
			pui8Data[ui32Idx] = pui8Data_Rev[ui32Idx];
		}
		//
		// Send the response.
		//
		//  	 UARTprintf("sending COM data to UDP port!\n");
		UARTprintf("ip adrr_dest:%x\n",piphost->addr);
		//  	UARTprintf("ip adrr_souce:%x\n",ip_current_src_addr()); //piphost

		//	 	udp_send(UDPpcb, p); //49156
		udp_sendto(UDPpcb, p, piphost, g_sParameters.sPort->ui16TelnetRemotePort); //IP_ADDR_ANY
		//	     udp_sendto(UDPpcb, p, piphost, 1444); //IP_ADDR_ANY
...
TelnetHandler(void)
{
	...
	//
	//    if(((Switch_LAN_to_COM ==1 ) &&(Switch_WiFi_to_COM ==0)))
	if(((Switch_LAN_to_COM ==1 ) &&(Switch_WiFi_to_COM ==0))||((Switch_LAN_to_COM ==0 ) &&(Switch_WiFi_to_COM ==1)))

	{
		//        pState = &g_sModbusSession;
		//    	UARTprintf("\n Switch_LAN_to_COM = %d\n",Switch_LAN_to_COM);
		for(i32Loop = 0; i32Loop < 1; i32Loop++)
			//for(i32Loop = 0; i32Loop < 2; i32Loop++)		//chuck
		{
			//
			...
			{
				RingBufFlush(&Data_From_COM_To_LAN);
				//RingBufFlush(&Data_From_COM1_To_LAN);
				continue;
			}

#if defined(CONFIG_RFC2217_ENABLED)
			//      if(g_sParameters.sPort->ucRFC2217 == RFC2217_en)
			if(g_sParameters.sPort->ui8Flags & RFC2217_SET == RFC2217_SET)
			{
				//
			...
			while(!SerialSendFull(i32Loop))
			{
				...
				//
				if(!((Switch_LAN_to_COM ==0 ) &&(Switch_WiFi_to_COM ==1)))
			 ...
			 */
			//            if(RingBufUsed(&Data_From_COM1_To_LAN)&&		//chuck
			//             tcp_sndbuf(pState->pConnectPCB) &&
			//             (pState->pConnectPCB->snd_queuelen < TCP_SND_QUEUELEN))
			if(RingBufUsed(&Data_From_COM_To_LAN)&&
					tcp_sndbuf(pState->pConnectPCB) &&
					(pState->pConnectPCB->snd_queuelen < TCP_SND_QUEUELEN))

			{
				/*
              	if(
              		...
              		((g_sParameters.sPort[COM_Port].ui8Flags &
              				PORT_PROTOCOL_MODBUS) != PORT_MODBUS_RTU_TCP) &&
    	              		((g_sParameters.sPort[COM_Port].ui8Flags &
    	              				PORT_PROTOCOL_MODBUS) != PORT_MODBUS_RTU_RTU) && (g_sParameters.sPort[COM_Port].ui32BaudRate==9600)
          					)
  			  // delay some time;
				...
				//  i32Count = (int32_t)RingBufUsed(&Data_From_COM1_To_LAN);		//chuck
				i32Count = (int32_t)RingBufUsed(&Data_From_COM_To_LAN);
				// 	UARTprintf("\n tcp_sndbuf(pState->pConnectPCB):%d\n",tcp_sndbuf(pState->pConnectPCB));
				//  	UARTprintf("\n i32Count:%d\n",i32Count);
						...
						(pState->pConnectPCB->snd_queuelen < TCP_SND_QUEUELEN))
				{
					...
					while(i32Count && (i32Index < sizeof(pui8Temp)))
					{
						...
						//pui8Temp[i32Index] = RingBufReadOne(&Data_From_COM1_To_LAN);	//chuck
						pui8Temp[i32Index] = RingBufReadOne(&Data_From_COM_To_LAN);
						i32Index++;
						i32Count--;
					}
					//
	...
	else if((Switch_LAN_to_COM ==1 ) &&(Switch_WiFi_to_COM ==1) ) // wifi to com and lan to com simultaneously;
	{
		//        pState = &g_sModbusSession;
		//    	UARTprintf("\n Switch_LAN_to_COM = %d\n",Switch_LAN_to_COM);
		for(i32Loop = 0; i32Loop < 1; i32Loop++)
...
#endif
			//    	            UARTprintf("\n vv:%d\n",WiFi_to_COM_TX_Finish);
			//
			// While space is available in the serial output queue, process the
			// pbufs received on the telnet interface.
			//
			while(!SerialSendFull(i32Loop) &&(WiFi_to_COM_TX_Finish == true))
			{
				//
				// Pop a pbuf off of the rx queue, if one is available, and we are
				// not already processing a pbuf.
					...
					(pState->pConnectPCB->snd_queuelen < TCP_SND_QUEUELEN))
			{
						...
						(pState->pConnectPCB->snd_queuelen < TCP_SND_QUEUELEN))
				{
					...
					while(i32Count && (i32Index < sizeof(pui8Temp)))
					{
						//    pui8Temp[i32Index] = SerialReceive(pState->ui32SerialPort);
						pui8Temp[i32Index] = RingBufReadOne(&Data_From_COM_To_LAN);
						//    	                    	UARTprintf("%02X ",pui8Temp[i32Index]);
						// send com data to LAN simultaneously;
						//    xQueueReceive( g_Que_COM_LAN_WiFi, &pui8Temp[i32Index], 0 );
						//   SerialSend(WiFi_Port, pui8Temp[i32Index]);
						//	xQueueSendToBack( g_Que_COM_LAN_WiFi, ( void * ) &pui8Temp[i32Index], ( portTickType ) 0 ); // sync;
						i32Index++;
						i32Count--;
					}
					//    	                    UARTprintf("\n tcp fill end\n");
...
TelnetHandler_1(void)
{
...

	{
		...
		//    	UARTprintf("\n Switch_LAN_to_COM = %d\n",Switch_LAN_to_COM);
		// for(i32Loop = 0; i32Loop < 1; i32Loop++)
		for(i32Loop = 1; i32Loop < 2; i32Loop++)		//chuck
		{
			//
			// Initialize the state pointer.
			//
...
void TelnetHandler_2(void)
{
...

	{
		...
		//    	UARTprintf("\n Switch_LAN_to_COM = %d\n",Switch_LAN_to_COM);
		// for(i32Loop = 0; i32Loop < 1; i32Loop++)
		for(i32Loop = 2; i32Loop < 3; i32Loop++)		//chuck
		{
			//
			// Initialize the state pointer.
			//
			...
			{
				//RingBufFlush(&Data_From_COM_To_LAN);
				RingBufFlush(&Data_From_COM2_To_LAN);
				continue;
			}
            ...
            if(SerialReceiveAvailable(pState->ui32SerialPort) &&
               ...
               (pState->pConnectPCB->snd_queuelen < TCP_SND_QUEUELEN))
			 */
			if(RingBufUsed(&Data_From_COM2_To_LAN)&&		//chuck
					tcp_sndbuf(pState->pConnectPCB) &&
					(pState->pConnectPCB->snd_queuelen < TCP_SND_QUEUELEN))
				//			if(RingBufUsed(&Data_From_COM_To_LAN)&&
				//					tcp_sndbuf(pState->pConnectPCB) &&
...

			{
				...
				//
				//      i32Count = (int32_t)SerialReceiveAvailable(pState->ui32SerialPort);
				i32Count = (int32_t)RingBufUsed(&Data_From_COM2_To_LAN);		//chuck
				//i32Count = (int32_t)RingBufUsed(&Data_From_COM_To_LAN);
				// 	UARTprintf("\n tcp_sndbuf(pState->pConnectPCB):%d\n",tcp_sndbuf(pState->pConnectPCB));
				//  	UARTprintf("\n i32Count:%d\n",i32Count);
				if(tcp_sndbuf(pState->pConnectPCB) < i32Count)
						...
						(pState->pConnectPCB->snd_queuelen < TCP_SND_QUEUELEN))
				{
					...
					{
						//     pui8Temp[i32Index] = SerialReceive(pState->ui32SerialPort);
						pui8Temp[i32Index] = RingBufReadOne(&Data_From_COM2_To_LAN);	//chuck
						//pui8Temp[i32Index] = RingBufReadOne(&Data_From_COM_To_LAN);
						i32Index++;
						i32Count--;
					}
...
TelnetNotifyLinkStatus(bool bLinkStatusUp)
{
	...
	for(i32Port = 0; i32Port < MAX_S2E_PORTS; i32Port++)	//chuck
	{
		g_sTelnetSession[i32Port].bLinkLost = true;
	}
	//g_sTelnetSession[0].bLinkLost = true;
}
